...
$this->params['breadcrumbs'][] = ['label' => 'Вакансии', 'url' => ['/vacancies']];
?>
...
<div class="vacancy-container width-block">
    <div class="article">
        <h1><?= $page->title ?></h1>
        <p><?= $page->teaser ?></p>
    </div>
        ...
        <?php if ($vacancy) { ?>
	        <?php foreach ($vacancy as $item) { ?>
                <div class="slider-article">
                    ...
                    <h3><a href="/vacancy/<?= $item->alias ?>"><?= $item->title ?></a></h3>
                </div>
	        <?php } ?>
        <?php } ?>
    </div>
    <div class="page-transition">
        <a href="<?= $_SERVER['HTTP_REFERER'] ?>" class="link-prev">Назад</a>
        <a href="/articles" class="link-next">Далее - «Статьи»</a>
    </div>
</div>
</div>